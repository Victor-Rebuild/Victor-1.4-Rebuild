...
namespace Vector {
  ...
  const s32 FACE_DISPLAY_WIDTH = Factory::IsXray() ? 160: 184;
  const s32 FACE_DISPLAY_HEIGHT = Factory::IsXray() ? 80: 96;
  ...
  const u16 DEFAULT_CAMERA_RESOLUTION_WIDTH  =  Factory::IsXray() ? 800 : 640;
  const u16 DEFAULT_CAMERA_RESOLUTION_HEIGHT =  Factory::IsXray() ? 600 : 360;

  const u16 CAMERA_SENSOR_RESOLUTION_WIDTH  = Factory::IsXray() ? 1600: 1280;
  const u16 CAMERA_SENSOR_RESOLUTION_HEIGHT = Factory::IsXray() ? 1200: 720;
